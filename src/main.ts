...
class DuelGame {
  ...
  private showRenderError(error: unknown) {
    const header = `<h1>DIMENSIONAL DUEL</h1><p class='sub'>Severe Sci-Fi Dimensional Combat Simulation</p>`;
  ...
  private setupThree() {
    ...
    if (!c) throw new Error('Render container was not created.');
    ...
    const floor = new this.THREE.Mesh(
      new this.THREE.PlaneGeometry(80, 80),
      new this.THREE.MeshStandardMaterial({ color: 0x101820, roughness: 0.6, metalness: 0.4 })
    );
    floor.rotation.x = -Math.PI / 2;
    this.scene.add(floor);

    this.playerMesh = new this.THREE.Mesh(
      new this.THREE.SphereGeometry(0.5, 18, 18),
      new this.THREE.MeshStandardMaterial({ color: 0x4cd6ff, emissive: 0x001f2d })
    );
    this.rivalMesh = new this.THREE.Mesh(
      new this.THREE.OctahedronGeometry(0.65),
      new this.THREE.MeshStandardMaterial({ color: 0xff5677, emissive: 0x330910 })
    );
  ...
  private tryShoot(shooter: Entity, target: Entity, weapon: WeaponType, dist: number, trigger: boolean) {
    ...
    if (weapon === 'AR') {
      ...
      let total = 0;
      for (let i = 0; i < 8; i++) if (Math.random() > w.spread) total += w.damage;
  ...
  private spawnDrone() {
    ...
    const m = new this.THREE.Mesh(
      new this.THREE.TorusKnotGeometry(0.28, 0.1, 50, 8),
      new this.THREE.MeshStandardMaterial({ color: 0xffc45c, emissive: 0x553300 })
    );
    m.userData.pos = pos;
    this.scene.add(m);
    this.drones.push(m);
