...
A browser-playable sci-fi dimensional combat prototype implemented with TypeScript and Three.js rendering via ES-module CDN loading (GitHub Pages compatible in this environment), structured for expansion toward the full campaign.
...
- Internet access to load the Three.js ES module at runtime
...
- `index.html` - app entry for module-based runtime load.
...
## Rendering Startup Reliability

- The game now loads Three.js via ES-module dynamic import from `three.module.js` (instead of relying on a removed global `three.min.js` path).
- If Three.js fails to load or renderer initialization fails, the app shows an on-screen error panel with recovery actions instead of throwing an uncaught crash.
