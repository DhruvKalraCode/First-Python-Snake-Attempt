...
class DimensionalMath {
    static zeros(d) { return Array.from({ length: d }, () => 0); }
    static dist(a, b) {
        return Math.sqrt(a.reduce((sum, v, i) => sum + (v - b[i]) ** 2, 0));
    }
    static move(pos, axis, dir, speed) {
        const n = [...pos];
        n[axis] += dir * speed;
        return n;
    }
...
class DuelGame {
    constructor(THREE) {
        ...
        this.playerWeapon = 'AR';
        this.rivalWeapon = 'Shotgun';
        this.drones = [];
        this.stars = [];
        this.input = new Set();
        this.player = { pos: [0], hp: 100, cooldown: 0 };
        this.rival = { pos: [6], hp: 100, cooldown: 0 };
        this.wave = 1;
        this.resultText = '';
        this.unlocked = 1;
        this.sensitivity = 1;
    ...
    renderUI() {
        ...
        if (this.screen === 'title') {
            ...
            document.getElementById('sens').oninput = (e) => {
                this.sensitivity = Number(e.target.value);
    ...
    showRenderError(error) {
        ...
        this.screen = 'result';
        this.app.innerHTML = `<div class='panel'>${header}<h2>Rendering Initialization Failed</h2><p class='error'>${message}</p><p>Please reload the page. If this persists on GitHub Pages, verify module CDN reachability.</p><button id='retry'>Retry Level</button><button id='levels'>Back to Levels</button></div>`;
        this.bind('retry', () => this.startLevel());
        this.bind('levels', () => { this.screen = 'levelSelect'; this.renderUI(); });
    }
    setupThree() {
        ...
        if (!c)
            throw new Error('Render container was not created.');
        this.scene = new this.THREE.Scene();
        this.scene.background = new this.THREE.Color(this.selectedLevel.backdrop);
        this.camera = new this.THREE.PerspectiveCamera(72, window.innerWidth / window.innerHeight, 0.1, 300);
        this.camera.position.set(0, 8, 16);
        this.renderer = new this.THREE.WebGLRenderer({ antialias: true });
        this.renderer.setSize(window.innerWidth, window.innerHeight);
        c.appendChild(this.renderer.domElement);
        const hemi = new this.THREE.HemisphereLight(0xbbe2ff, 0x0a0a16, 1.6);
        const dir = new this.THREE.DirectionalLight(0xffffff, 1.5);
        dir.position.set(10, 12, 5);
        this.scene.add(hemi, dir);
        const floor = new this.THREE.Mesh(new this.THREE.PlaneGeometry(80, 80), new this.THREE.MeshStandardMaterial({ color: 0x101820, roughness: 0.6, metalness: 0.4 }));
        floor.rotation.x = -Math.PI / 2;
        this.scene.add(floor);
        this.playerMesh = new this.THREE.Mesh(new this.THREE.SphereGeometry(0.5, 18, 18), new this.THREE.MeshStandardMaterial({ color: 0x4cd6ff, emissive: 0x001f2d }));
        this.rivalMesh = new this.THREE.Mesh(new this.THREE.OctahedronGeometry(0.65), new this.THREE.MeshStandardMaterial({ color: 0xff5677, emissive: 0x330910 }));
        this.scene.add(this.playerMesh, this.rivalMesh);
        this.drones = [];
        this.stars = [];
        for (let i = 0; i < 120; i++) {
            const s = new this.THREE.Mesh(new this.THREE.SphereGeometry(0.05, 6, 6), new this.THREE.MeshBasicMaterial({ color: 0xffffff }));
            s.position.set((Math.random() - 0.5) * 90, Math.random() * 30 + 6, (Math.random() - 0.5) * 90);
            this.scene.add(s);
            this.stars.push(s);
        }
    }
    ...
    tickGameplay(dt) {
        ...
        const map = [['a', 0, -1], ['d', 0, 1], ['w', 1, 1], ['s', 1, -1], ['q', 2, -1], ['e', 2, 1], ['r', 3, 1], ['f', 3, -1], ['t', 4, 1], ['g', 4, -1]];
        for (const [key, axis, dir] of map) {
            if (axis < this.selectedLevel.dimension && this.input.has(key)) {
                this.player.pos = DimensionalMath.move(this.player.pos, axis, dir, speed * this.sensitivity);
            }
        }
        ...
        if (this.drones.length < this.wave + 1)
            this.spawnDrone();
        this.drones.forEach((d) => {
            ...
            if (dd < 10 && Math.random() < dt * 0.8)
                this.player.hp -= 2;
            const [x, y, z] = Projection.to3D(d.userData.pos);
        ...
        this.camera.position.lerp(new this.THREE.Vector3(px + 6, py + 8, pz + 9), 0.06);
    ...
    tryShoot(shooter, target, weapon, dist, trigger) {
        if (!trigger || shooter.cooldown > 0)
            return;
    ...
    spawnDrone() {
        ...
        const m = new this.THREE.Mesh(new this.THREE.TorusKnotGeometry(0.28, 0.1, 50, 8), new this.THREE.MeshStandardMaterial({ color: 0xffc45c, emissive: 0x553300 }));
        m.userData.pos = pos;
        this.scene.add(m);
        this.drones.push(m);
    }
...
async function boot() {
    ...
    try {
        const threeUrl = 'https://unpkg.com/three@0.164.1/build/three.module.js';
        const THREE = await import(threeUrl);
        new DuelGame(THREE);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        app.innerHTML = `<div class='panel'><h1>DIMENSIONAL DUEL</h1><p class='sub'>Severe Sci-Fi Dimensional Combat Simulation</p><h2>Startup Error</h2><p class='error'>Unable to load Three.js module. ${message}</p><p>Check network access to CDN module URLs and reload.</p></div>`;
    }
}
void boot();
